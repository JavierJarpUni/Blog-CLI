...
public class BlogManager {
    ...
    private void savePosts() {
        try (Writer writer = Files.newBufferedWriter(Paths.get(filename))) {
            Gson gson = new Gson();
            // Convert the list of BlogPost objects to a list of dictionaries (maps) and then to JSON
    ...
    private void loadPosts() {
        try {
            if (Files.exists(Paths.get(filename))) {
                ...
                Gson gson = new Gson();
                // Deserialize the JSON into a list of maps, then convert each map back into a BlogPost
                List<Map<String, Object>> postMaps = gson.fromJson(json, List.class);
